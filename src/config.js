...
export const config = Object.freeze({
  ...
  githubBackupEnabled: toBoolean(process.env.GITHUB_BACKUP_ENABLED, true),
  githubBackupRepo: (process.env.GITHUB_BACKUP_REPO || 'italoeteixeira/chatbotgpt').trim(),
  githubBackupToken: String(process.env.GITHUB_BACKUP_TOKEN || ''),
  githubBackupBranches: parseCsv(process.env.GITHUB_BACKUP_BRANCHES, ['main', 'homologacao']),
  githubBackupUpdateReadme: toBoolean(process.env.GITHUB_BACKUP_UPDATE_README, true),
  githubBackupRunTestSuite: toBoolean(process.env.GITHUB_BACKUP_RUN_TEST_SUITE, true),
  githubBackupAutoRollback: toBoolean(process.env.GITHUB_BACKUP_AUTO_ROLLBACK, true),
  backupSchedulerMode: (process.env.BACKUP_SCHEDULER_MODE || 'validated_github').trim().toLowerCase(),
  backupSchedulerIntervalHours: toPositiveInt(process.env.BACKUP_SCHEDULER_INTERVAL_HOURS, 24),
